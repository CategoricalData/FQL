...
//		vars.addAll(rhs.vars());
	...
	private final Set<Pair<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>, Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>>> seen = new HashSet<>();

	private Map<C, List<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>>> AC_symbols;

	private final KBOptions options;

	// private final Map<T, C> min = new HashMap<>();
...
//		}
	...
	private void compose() {
		...
		do {
			...
			for (Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>> r : R) {
				Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> R0 = new HashSet<>(R);
				R0.remove(r);
	...
	private Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> allcps2(
		...
		Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> ret = (new HashSet<>());

		Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> E0 = (new HashSet<>(E));
		E0.add(ab);
		E0.add(ab.reverse12());
	...
	private void simplify() {
		Map<KBExp<C, V>, KBExp<C, V>> cache = new HashMap<>();

		List<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> newE = new LinkedList<>();
		Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> newE2 = new HashSet<>();
		for (Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>> e : E) {
	...
	private boolean strongGroundJoinable(BiFunction<Chc<V, C>, Chc<V, C>, Boolean> gt, KBExp<C, V> s, KBExp<C, V> t,
			Map<V, T> ctx) {
		List<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> R0 = new LinkedList<>();
	...
	private Collection<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> reduce(
		...
		Set<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> p = new HashSet<>();
		for (Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>> e : set) {
			KBExp<C, V> lhs = red(this::gtX, new HashMap<>(), Util.append(E, G), R, e.first, e.third.values());
			KBExp<C, V> rhs = red(this::gtX, new HashMap<>(), Util.append(E, G), R, e.second, e.third.values());
			if (lhs.equals(rhs)) {
				continue;
			}
	...
	private boolean allCpsConfluent(boolean print, boolean ground) {
		for (Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>> e : E) {
			List<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> set = filterSubsumed(
					reduce(allcps2(new HashSet<>(), e)));
			if (!allCpsConfluent(print, ground, "equation " + e, set)) {
				return false;
			}
		}
		for (Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>> e : R) {
			List<Triple<KBExp<C, V>, KBExp<C, V>, Map<V, T>>> set = filterSubsumed(reduce(allcps(new HashSet<>(), e)));
